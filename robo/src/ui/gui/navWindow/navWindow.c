...
static void touchEvent(void *node, uint16_t x, uint16_t y) {
    ...
    if (node_containsPoint((node_t *)&window->nextButton, x, y)) {
        ...
        incDec = (incDec == 1) ? -1 : 1;
        text_writeFormatAtPoint(f_12x16, 0, 100, LEFT, "%s", (incDec == 1) ? "inc" : "dec");
    }
    text_writeFormatAtPoint(f_12x16, 0, 60, LEFT, "(Kp, Ki) = (%d, %d)    ", pid1, pid2);
}
...
navWindow_t navWindow_create(void) {
    ...
    window.pidButton1 = button_create("  Kp  ", f_12x16, color16(0xDDDDDD));
